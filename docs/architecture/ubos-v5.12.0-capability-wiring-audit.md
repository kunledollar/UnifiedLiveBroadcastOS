...
## Step 5 Local Native Runtime Connection and Control Room Recording Activation

Step 5 keeps the product on **UBOS v5.12.0** and connects the existing Control Room recording surface to the server-side native runtime path. This is not a new engine or decorative UI; it is a local native recording path with browser Program media handoff to the API/server host and FFmpeg finalization.

### Native runtime API endpoints

| Endpoint | Purpose |
| --- | --- |
| `GET /api/native-runtime/status` | Returns authoritative server runtime state: host connection, FFmpeg path/version/state, FFprobe path/version/state, recording readiness, active state, last artifact, and last failure. |
| `POST /api/native-runtime/recording/finalize` | Accepts the current Program WebM handoff from the Control Room, runs server-side FFmpeg to H.264/AAC MP4, validates the artifact with FFprobe, and returns the artifact result or a safe failure. |

### Program media handoff method

The implemented handoff is **browser MediaStream → MediaRecorder WebM chunks → controlled API upload → server-side FFmpeg transcode**.

| Concern | Implementation |
| --- | --- |
| Video codec/framing | Browser-produced WebM video chunks from the actual Program capture stream. |
| Audio codec/framing | Browser-produced WebM audio chunks when audio tracks are present; server records explicit no-audio mode otherwise. |
| Timestamps | Browser MediaRecorder timestamps and FFmpeg demux/mux timestamps. |
| Backpressure | Chunks are bounded by the operator recording session and uploaded once for native finalization; FFmpeg stderr is bounded. |
| Disconnect | Runtime status polling reports host disconnect; finalization returns safe failure if upload or server host fails. |
| Cancellation/stop | Existing Stop control stops browser chunk capture, then server finalization begins. |
| Shutdown | Server-side FFmpeg process is spawned with `shell: false`, timeout, and clean exit handling. |
| Maximum buffered data | Browser chunks are held for the active operator session only; production hardening should add a configured maximum before GA. |

### Control Room native recording flow

Control Room native recording now derives readiness from the authoritative runtime status API plus Program media availability. Start Native is enabled only when FFmpeg/FFprobe are available, Program media is on Program, the output path is writable, the adapter is healthy, and no active session conflicts. Stop Native is enabled only while native recording is active. Browser MediaRecorder remains visible as the labeled fallback.

### Streaming panel correction

The streaming panel no longer uses literal permanent disabled buttons. It polls `/api/native-runtime/status`, derives `canStart` and `canStop`, requires a valid Custom RTMP/RTMPS URL and `secret://` stream key reference, and keeps RTMP blocked until an approved destination exists.

### Local Windows validation commands

Run these on the maintainer Windows UBOS host from the repo root:
...
1. Add/start a camera, screen, or media source.
2. Take it to Program.
3. Open Recording operations.
4. Confirm Native FFmpeg Recording shows FFmpeg and FFprobe available.
5. Click **Start Native**.
6. Wait for active elapsed time.
7. Click **Stop Native**.
8. Record the returned artifact path, file size, duration, video codec, audio codec, playable result, and clean FFmpeg exit.

### Step 5 status

| Item | Status |
| --- | --- |
| API runtime endpoint | Implemented in Next server API route. |
| Windows runtime discovery result | Must be captured on maintainer Windows host; Codex container is not authoritative for the Windows PATH. |
| Program media handoff | Implemented through Program capture WebM upload to server-side FFmpeg finalization. |
| Control Room start result | Wired; requires Windows host validation with Program media present. |
| Control Room stop result | Wired; stop triggers native finalization and FFprobe verification. |
| Artifact path/size/duration/codecs | Returned by server route after FFprobe PASS. |
| Browser fallback | Preserved and explicitly labeled as browser-local recording. |
| Remaining RTMP blocker | Approved destination and `secret://` stream key reference; RTMP not enabled yet. |

## Step 5 release decision

**PARTIAL until Windows operator validation is run.** The Control Room is now connected to server native runtime status and native recording finalization, but PASS requires the maintainer to initiate and stop a native recording through the Control Room on the Windows host and verify the MP4 artifact with FFprobe. Do not proceed to RTMP until this passes.
