...
export {
  MEDIA_ENCODER_COMMAND_TYPES,
  MEDIA_ENCODER_EVENTS,
  MEDIA_ENCODER_FOUNDATION_PROCESSOR_ORDER,
  MEDIA_ENCODER_FOUNDATION_VERSION,
  MEDIA_ENCODER_OUTPUT_KEYS,
  MEDIA_ENCODER_WATCHDOG_INCIDENTS,
  MediaEncoderFoundationEngine,
  MediaEncoderFoundationError,
  MediaEncoderFoundationProcessor,
  SyntheticAudioEncoderBackend,
  SyntheticVideoEncoderBackend,
  createAudioEncoderConfiguration,
  createMediaEncoderCommandHandlers,
  createMediaEncoderFoundationEngine,
  createMediaEncoderFoundationProcessor,
  createSyntheticAudioEncoderBackend,
  createSyntheticVideoEncoderBackend,
  createVideoEncoderConfiguration,
  validateAudioEncoderConfiguration,
  validateVideoEncoderConfiguration,
  type AudioEncodeInputBlock,
  type AudioEncodeInputBlockSnapshot,
  type AudioEncoderBackend,
  type AudioEncoderBackendSnapshot,
  type AudioEncoderCodec,
  type AudioEncoderConfiguration,
  type AudioEncoderConfigurationSnapshot,
  type AudioEncoderProfile,
  type BackpressureLevel,
  type BackpressureSnapshot,
  type BitrateMode,
  type CodecConfigurationPacketSnapshot,
  type EncodedAudioVideoCorrelationSnapshot,
  type EncodedMediaPacket,
  type EncodedMediaPacketSnapshot,
  type EncodedPacketLease,
  type EncodedPacketLeaseSnapshot,
  type EncoderBackendDescriptor,
  type EncoderBackpressureSnapshot,
  type EncoderDrainSnapshot,
  type EncoderErrorType,
  type EncoderFlushSnapshot,
  type EncoderImplementationClass,
  type EncoderInputQueueSnapshot,
  type EncoderPacketQueueSnapshot,
  type EncoderQualityTier,
  type EncoderRateControlStateSnapshot,
  type ExtradataGenerationPolicy,
  type FrameClassification,
  type InputOverflowPolicy,
  type KeyframeReason,
  type MediaEncodePlan,
  type MediaEncodePlanSnapshot,
  type MediaEncodeRequest,
  type MediaEncodeRequestSnapshot,
  type MediaEncoderCommandType,
  type MediaEncoderConfigurationTransaction,
  type MediaEncoderConfigurationTransactionSnapshot,
  type MediaEncoderEngineSnapshot,
  type MediaEncoderEventType,
  type MediaEncoderHealthSnapshot,
  type MediaEncoderOutputBinding,
  type MediaEncoderOutputBindingSnapshot,
  type MediaEncoderOutputRole,
  type MediaEncoderSessionDefinition,
  type MediaEncoderSessionDefinitionSnapshot,
  type MediaEncoderSessionState,
  type MediaEncoderSessionStateSnapshot,
  type MediaEncoderTelemetrySnapshot,
  type MediaEncoderValidationReport,
  type MediaEncoderWatchdogIncident,
  type MediaType,
  type PacketOverflowPolicy,
  type PacketOwner,
  type QueuePolicy,
  type ReconfigurationPolicy,
  type SafeMetadata,
  type SessionStartupPolicy,
  type SyntheticBackendFaults,
  type VideoEncodeInputFrame,
  type VideoEncodeInputFrameSnapshot,
  type VideoEncoderBackend,
  type VideoEncoderBackendSnapshot,
  type VideoEncoderCodec,
  type VideoEncoderConfiguration,
  type VideoEncoderConfigurationSnapshot,
  type VideoEncoderGopState,
  type VideoEncoderGopStateSnapshot,
