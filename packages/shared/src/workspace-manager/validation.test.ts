...
const assertEqual = (actual: unknown, expected: unknown, message?: string) => { if (actual !== expected) throw new Error(message ?? `Expected ${String(expected)}, received ${String(actual)}`); };
const assertThrows = (fn: () => void, message: string) => { let threw = false; try { fn(); } catch { threw = true; } if (!threw) throw new Error(message); };
...
assertEqual(workspaceZoneDefinitions['left-dock'].defaultSize, 260);
assertEqual(workspaceZoneDefinitions['left-dock'].minSize, 220);
assertEqual(workspaceZoneDefinitions['left-dock'].maxSize, 360);
...
assertEqual(workspaceZoneDefinitions['right-dock'].defaultSize, 320);
assertEqual(workspaceZoneDefinitions['right-dock'].minSize, 260);
assertEqual(workspaceZoneDefinitions['right-dock'].maxSize, 420);
assertEqual(workspaceZoneDefinitions['right-dock'].collapsedSize, 0);
assertEqual(workspaceZoneDefinitions['bottom-workspace'].defaultSize, 260);
...
assertEqual(clampZoneSize('left-dock', 100), 220);
assertEqual(clampZoneSize('left-dock', 500), 360);
assertEqual(clampZoneSize('bottom-workspace', 300), 300);

assertEqual(getResponsiveCollapsedZones(1920).length, 0);
assertOk(getResponsiveCollapsedZones(1439).includes('right-dock'), 'right-dock collapses below 1440px');
assertOk(!getResponsiveCollapsedZones(1439).includes('left-dock'));
assertOk(getResponsiveCollapsedZones(1199).includes('left-dock'), 'left-dock collapses below 1200px');
...
for (const panel of defaults) {
  assertEqual(validatePanelDefinition(panel).length, 0, `default panel "${panel.id}" must validate`);
...
assertEqual(centerPanels[0]?.id, WORKSPACE_PANEL_IDS.programMonitor, 'zone panels sorted by priority');
assertEqual(centerPanels[1]?.id, WORKSPACE_PANEL_IDS.previewMonitor);
assertOk(registry.getPanelsForZone('left-dock').some((panel) => panel.id === WORKSPACE_PANEL_IDS.scenes));
...
assertOk(validatePanelDefinition(badZone).some((issue) => issue.code === 'PANEL_DEFAULT_ZONE_INVALID'));
assertThrows(() => registry.registerPanel(badZone), 'invalid zone must be rejected');

const notAllowedDefault = { ...defaults[2]!, id: 'zone-mismatch', defaultZone: 'right-dock' as WorkspaceZoneId, allowedZones: ['left-dock'] as WorkspaceZoneId[] };
assertOk(validatePanelDefinition(notAllowedDefault).some((issue) => issue.code === 'PANEL_DEFAULT_ZONE_NOT_ALLOWED'));

const withRuntimeObject = { ...defaults[2]!, id: 'runtime-object', mediaStream: { attach: () => undefined } } as unknown as WorkspacePanelDefinition;
assertOk(
  validatePanelDefinition(withRuntimeObject).some((issue) => issue.code === 'PANEL_NOT_SERIALIZABLE'),
  'runtime media objects must be rejected from the registry',
);
assertThrows(() => registry.registerPanel(withRuntimeObject), 'runtime objects must not be registered');
...
assertThrows(() => registry.togglePanelVisibility(WORKSPACE_PANEL_IDS.programMonitor), 'program monitor is not closable');
assertThrows(() => registry.togglePanelCollapsed(WORKSPACE_PANEL_IDS.programMonitor), 'program monitor is not collapsible');
...
assertOk(registry.getPanelsForZone('bottom-workspace').some((panel) => panel.id === WORKSPACE_PANEL_IDS.scenes));
assertThrows(() => registry.movePanelToZone(WORKSPACE_PANEL_IDS.scenes, 'external-monitor'), 'zone must be in allowedZones');
assertThrows(() => registry.movePanelToZone('missing-panel', 'left-dock'), 'unknown panel must throw');
...
for (const preset of Object.values(workspacePresets)) {
  for (const panelId of [...preset.visiblePanels, ...preset.collapsedPanels, ...preset.hiddenPanels, preset.activeBottomTab]) {
    assertOk(knownPanelIds.has(panelId), `preset "${preset.id}" references unknown panel "${panelId}"`);
  }
  assertOk(preset.visiblePanels.includes(WORKSPACE_PANEL_IDS.programMonitor), `preset "${preset.id}" must keep Program visible`);
  assertOk(preset.visiblePanels.includes(WORKSPACE_PANEL_IDS.previewMonitor), `preset "${preset.id}" must keep Preview visible`);
}
...
assertOk(workspacePresets['solo-streamer'].hiddenPanels.includes(WORKSPACE_PANEL_IDS.routingMatrix), 'solo streamer hides routing');
assertOk(workspacePresets['solo-streamer'].hiddenPanels.includes(WORKSPACE_PANEL_IDS.pipelineInspector), 'solo streamer hides diagnostics');
assertEqual(workspacePresets['technical-director'].activeBottomTab, WORKSPACE_PANEL_IDS.routingMatrix);
assertEqual(workspacePresets['audio-engineer'].activeBottomTab, WORKSPACE_PANEL_IDS.audioMixer);
assertEqual(workspacePresets['graphics-operator'].activeBottomTab, WORKSPACE_PANEL_IDS.graphicsLibrary);
assertEqual(workspacePresets['replay-operator'].activeBottomTab, WORKSPACE_PANEL_IDS.replayTimeline);
assertEqual(workspacePresets['streaming-operator'].activeBottomTab, WORKSPACE_PANEL_IDS.systemStatus);
...
const brokenPreset = { ...workspacePresets.director, hiddenPanels: [WORKSPACE_PANEL_IDS.programMonitor] };
assertOk(validateWorkspacePreset(brokenPreset).some((issue) => issue.code === 'PRESET_MONITOR_HIDDEN'));
const conflictPreset = { ...workspacePresets.director, collapsedPanels: [WORKSPACE_PANEL_IDS.scenes], visiblePanels: [...workspacePresets.director.visiblePanels] };
assertOk(validateWorkspacePreset(conflictPreset).some((issue) => issue.code === 'PRESET_PANEL_STATE_CONFLICT'));
...
assertEqual(reference.zones['left-dock'].rect.width, 260);
assertEqual(reference.zones['right-dock'].rect.width, 320);
assertEqual(reference.zones['bottom-workspace'].rect.height, 260);
assertEqual(reference.zones['center-stage'].rect.width, 1920 - 72 - 260 - 320);
...
assertOk(!rectsOverlap(reference.zones['bottom-workspace'].rect, reference.programRect), 'bottom workspace never covers Program');
assertOk(!rectsOverlap(reference.zones['left-dock'].rect, reference.previewRect), 'docks never cover Preview');

// Center-stage receives all freed space when docks collapse (compact preset).
const compact = calculateWorkspaceLayout({ viewportWidth: 1920, viewportHeight: 1080, preset: workspacePresets.compact });
assertEqual(compact.zones['left-dock'].rect.width, 0);
assertEqual(compact.zones['right-dock'].rect.width, 0);
assertEqual(compact.zones['bottom-workspace'].rect.height, 42, 'collapsed bottom workspace keeps its tab bar');
assertEqual(compact.zones['center-stage'].rect.width, 1920 - 72, 'center-stage absorbs freed dock space');
assertOk(compact.zones['center-stage'].rect.height > reference.zones['center-stage'].rect.height, 'center-stage absorbs freed bottom space');
assertEqual(validateLayoutResult(compact).length, 0);

// Responsive collapse thresholds.
const at1439 = calculateWorkspaceLayout({ viewportWidth: 1439, viewportHeight: 900, preset: workspacePresets.director });
...
const at1199 = calculateWorkspaceLayout({ viewportWidth: 1199, viewportHeight: 800, preset: workspacePresets.director });
...
assertEqual(at1500.zones['right-dock'].collapsed, true, 'right-dock force-collapses to protect center minimum');
...
const narrow = calculateWorkspaceLayout({ viewportWidth: 899, viewportHeight: 700, preset: workspacePresets.director });
assertEqual(narrow.monitorsStacked, true);
assertEqual(narrow.programRect.width, narrow.previewRect.width, 'stacked monitors share full center width');
assertEqual(narrow.programRect.x, narrow.previewRect.x);
assertOk(narrow.previewRect.y >= narrow.programRect.y + narrow.programRect.height, 'preview stacks below program');
...
assertOk(emphasized.programRect.width > emphasized.previewRect.width, 'program emphasis widens Program');
...
const operatorCollapsed = calculateWorkspaceLayout({ viewportWidth: 1920, viewportHeight: 1080, preset: workspacePresets.director, collapsedZones: ['left-dock', 'center-stage'] });
assertEqual(operatorCollapsed.zones['left-dock'].collapsed, true);
assertEqual(operatorCollapsed.zones['center-stage'].collapsed, false, 'center-stage may never collapse');
...
assertEqual(parsed?.panelStates.find((state) => state.panelId === WORKSPACE_PANEL_IDS.scenes)?.zone, 'bottom-workspace');

assertEqual(parseLayoutSnapshot('not json'), null);
assertEqual(parseLayoutSnapshot('{"version":99}'), null);
assertEqual(parseLayoutSnapshot('{"version":1,"activePresetId":"unknown","collapsedZones":[],"panelStates":[]}'), null);
...
assertEqual(restoreTarget.getPanelState(WORKSPACE_PANEL_IDS.scenes)?.zone, 'bottom-workspace', 'snapshot restores panel zones');
assertEqual(restoreTarget.getPanelState(WORKSPACE_PANEL_IDS.chat)?.visible, true, 'snapshot restores panel visibility');
assertEqual(restoreTarget.getPanelState(WORKSPACE_PANEL_IDS.programMonitor)?.visible, true, 'non-closable panels stay visible after restore');

// Snapshots with unknown panels are applied without error.
const foreignSnapshot = parseLayoutSnapshot(
  JSON.stringify({ version: 1, activePresetId: 'compact', collapsedZones: [], panelStates: [{ panelId: 'ghost-panel', zone: 'left-dock', visible: true, collapsed: false }], savedAt: new Date().toISOString() }),
);
applyLayoutSnapshot(restoreTarget, foreignSnapshot!);
assertEqual(restoreTarget.getPanel('ghost-panel'), undefined, 'unknown panels are skipped, never created');
