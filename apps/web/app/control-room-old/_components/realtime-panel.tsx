  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-slate-900/75 p-4 text-sm text-slate-200">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <span className={`rounded-full px-2 py-1 text-[11px] font-bold ${status === 'connected' ? 'bg-emerald-400/20 text-emerald-200' : status === 'api-offline' ? 'bg-slate-500/20 text-slate-200' : 'bg-amber-400/20 text-amber-100'}`}>
          {statusLabels[status]}
        </span>
      </div>
