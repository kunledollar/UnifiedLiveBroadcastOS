...
export function StreamingRuntimePanel({ state, dispatch }: { state: BrowserStreamingPanelState; dispatch: (action: StreamingPanelAction) => void }) {
  const busy = state.lifecycle === 'preparing' || state.lifecycle === 'connecting' || state.lifecycle === 'streaming' || state.lifecycle === 'reconnecting';
  return <div className="space-y-ubos-2">
    <BroadcastPanel>
      ...
      <p className="mb-3 rounded-ubos-sm border border-amber-400/40 bg-amber-400/10 p-2 text-ubos-caption text-amber-100">Browser mode stores streaming metadata and simulates lifecycle only. Real RTMP transport requires backend/native FFmpeg support through BrowserStreamingAdapter, FFmpegStreamingAdapter, or NativeDesktopStreamingAdapter.</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2"><button className="rounded-ubos-sm bg-emerald-400 px-2 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-950 disabled:opacity-50" disabled={busy} onClick={() => dispatch({ type: 'start' })}>Start Streaming</button><button className="rounded-ubos-sm bg-ubos-midnight px-2 py-2 text-xs font-black uppercase tracking-[0.12em] disabled:opacity-50" disabled={state.lifecycle !== 'streaming' && state.lifecycle !== 'reconnecting'} onClick={() => dispatch({ type: 'stop' })}>Stop Streaming</button></div>
      {state.error ? <p className="mt-2 text-ubos-caption text-ubos-error-text">{state.error}</p> : null}
      <dl className="mt-3 grid grid-cols-2 gap-2 text-sm"><dt className="text-ubos-fg-muted">Timer</dt><dd>{formatDuration(state.durationMs)}</dd><dt className="text-ubos-fg-muted">Bitrate estimate</dt><dd>{state.bitrateEstimateKbps} kbps</dd><dt className="text-ubos-fg-muted">Dropped frames</dt><dd>{state.droppedFrameEstimate}</dd><dt className="text-ubos-fg-muted">Adapters</dt><dd>{state.adapters.join(', ')}</dd></dl>
    </BroadcastPanel>
